...
// ============================================================================
// ROBUST DATE PARSING — supports all Excel/common date formats
// ============================================================================
function parseDate(value: any): Date | null {
  ...
  if (value instanceof Date) {
    ...
    if (!valid) console.warn('[parseDate] Invalid Date object:', value);
    return valid ? value : null;
  }

  // Excel serial number (days since 1900-01-00)
  if (typeof value === 'number') {
    if (value > 1 && value < 2958466) { // 1900-01-01 to 9999-12-31
      const date = new Date((value - 25569) * 86400 * 1000);
      const valid = !isNaN(date.getTime());
      console.debug(`[parseDate] serial ${value} → ${valid ? date.toISOString() : 'INVALID'}`);
      return valid ? date : null;
    }
    console.warn(`[parseDate] Number out of serial range: ${value}`);
    return null;
  }

  if (typeof value !== 'string') {
    console.warn(`[parseDate] Unsupported type: ${typeof value}`, value);
    return null;
  }
  ...
  // Helper: build date and validate
  ...
  // Separator-based patterns  (D/M/Y, D-M-Y, D.M.Y)
  const sepMatch = raw.match(/^(\d{1,4})([\\/\-\.])(\d{1,4})\2(\d{2,4})$/);
  if (sepMatch) {
    const a = parseInt(sepMatch[1], 10);
    const b = parseInt(sepMatch[3], 10);
    let c = parseInt(sepMatch[4], 10);
    if (c < 100) c += c < 50 ? 2000 : 1900;

    // Determine if a is a 4-digit year (YYYY/MM/DD)
    if (a > 31) {
      const d = makeDate(a, b, c);
      if (d) {
        console.debug(`[parseDate] YYYY/MM/DD "${raw}" → ${d.toISOString()}`);
        return d;
      }
    }

    // DD/MM/YYYY — try day first (most common in Morocco/France)
    if (a >= 1 && a <= 31 && b >= 1 && b <= 12) {
      const d = makeDate(c, b, a);
      if (d) {
        console.debug(`[parseDate] DD/MM/YYYY "${raw}" → ${d.toISOString()}`);
        return d;
      }
    }
    ...
    if (b >= 1 && b <= 31 && a >= 1 && a <= 12) {
      const d = makeDate(c, a, b);
      if (d) {
        console.debug(`[parseDate] MM/DD/YYYY "${raw}" → ${d.toISOString()}`);
        return d;
      }
    }
  }

  // ISO: YYYY-MM-DD (already covered by sepMatch but explicit fallback)
  const isoMatch = raw.match(/^(\d{4})-(\d{2})-(\d{2})/);
  if (isoMatch) {
    const d = makeDate(parseInt(isoMatch[1]), parseInt(isoMatch[2]), parseInt(isoMatch[3]));
    if (d) {
      console.debug(`[parseDate] ISO "${raw}" → ${d.toISOString()}`);
      return d;
    }
  }

  // Native Date.parse as last resort (handles "Jan 30 2026", etc.)
  const native = new Date(raw);
  if (!isNaN(native.getTime())) {
    console.debug(`[parseDate] native parse "${raw}" → ${native.toISOString()}`);
    return native;
  }

  console.warn(`[parseDate] FAILED to parse: "${raw}"`);
  return null;
}
...
// ============================================================================
// PDF GENERATION — smart pagination with no row splits
// ============================================================================
async function generatePdfWithSmartPagination(
  doc: jsPDF,
  content: PdfContentBlock[],
  opts: { margin: number; pageWidth: number; pageHeight: number }
): Promise<void> {
  ...
  const ensureSpace = (needed: number): boolean => {
    return cursorY + needed <= pageHeight - margin;
  };

  for (const block of content) {
    if (block.type === 'image') {
      const { imgData, imgWidth, imgHeight } = block;
      ...
      doc.addImage(imgData, 'PNG', margin, cursorY, usableWidth, h);
      cursorY += h + 8;

    } else if (block.type === 'table') {
      const { headers, rows, colWidths, rowHeight, headerHeight, headerBg, accentColor } = block;
      const totalTableWidth = colWidths.reduce((a: number, b: number) => a + b, 0);
      const tableX = margin;

      // Draw the table in chunks, never splitting a row across pages
      let firstChunk = true;

      const drawTableHeader = (y: number) => {
        // Header background
        doc.setFillColor(headerBg[0], headerBg[1], headerBg[2]);
        doc.roundedRect(tableX, y, totalTableWidth, headerHeight, 2, 2, 'F');

        doc.setFont('Helvetica', 'bold');
        doc.setFontSize(8);
        doc.setTextColor(accentColor[0], accentColor[1], accentColor[2]);

        let x = tableX + totalTableWidth;
        for (let i = 0; i < headers.length; i++) {
          x -= colWidths[i];
          doc.text(headers[i], x + colWidths[i] / 2, y + headerHeight / 2 + 2.5, { align: 'center' });
        }
        return y + headerHeight;
      };

      // First: ensure header fits
      if (!ensureSpace(headerHeight + rowHeight)) {
        addPage();
      }
      ...
      for (let ri = 0; ri < rows.length; ri++) {
        const row = rows[ri];
        const rh = row._height || rowHeight;

        // If this row doesn't fit, start a new page and redraw header
        if (!ensureSpace(rh) || tableY + rh > pageHeight - margin) {
          addPage();
          tableY = cursorY;
          tableY = drawTableHeader(tableY);
        }
        ...
        // Row background
        const bgColor = row._bg || (ri % 2 === 0 ? [255, 255, 255] : [248, 250, 252]);
        doc.setFillColor(bgColor[0], bgColor[1], bgColor[2]);
        doc.rect(tableX, tableY, totalTableWidth, rh, 'F');

        // Row border
        doc.setDrawColor(241, 245, 249);
        doc.setLineWidth(0.3);
        doc.line(tableX, tableY + rh, tableX + totalTableWidth, tableY + rh);
        ...
        for (let ci = 0; ci < row.cells.length; ci++) {
          ...
          const textColor = cell.color || [71, 85, 105];
          doc.setTextColor(textColor[0], textColor[1], textColor[2]);
          
          // Red border highlight for error cells
          if (cell.highlight === 'error') {
            doc.setDrawColor(239, 68, 68);
            doc.setLineWidth(0.8);
            doc.rect(x + 0.5, tableY + 0.5, colWidths[ci] - 1, rh - 1, 'S');
            doc.setDrawColor(241, 245, 249);
            doc.setLineWidth(0.3);
          }

          doc.text(String(cell.text || ''), x + colWidths[ci] / 2, tableY + rh / 2 + 2.5, { align: 'center' });
        }
...
type PdfContentBlock =
  | { type: 'image'; imgData: string; imgWidth: number; imgHeight: number }
  | { type: 'table'; headers: string[]; rows: any[]; colWidths: number[]; rowHeight: number; headerHeight: number; headerBg: number[]; accentColor: number[] }
...
export default function Statistics({ data, columns, allSheets }: StatisticsProps) {
  ...
  // FIX 1: Filter can target any date column (not just start date)
  const [filterDateCol, setFilterDateCol] = useState<string>(''); // which column to filter on
  ...
  // When startDateCol changes, default the filter column to it
  useEffect(() => {
    if (startDateCol && !filterDateCol) setFilterDateCol(startDateCol);
  }, [startDateCol]);
  ...
  const durationStats = useMemo(() => {
    ...
    type DurationRow = {
      ...
      formatted: string;
      status: 'ok' | 'missing' | 'invalid' | 'parse_error';
      parseError?: string;
    };

    const filterColTarget = filterDateCol || startDateCol;
    ...
    data.forEach((row, idx) => {
      ...
      const filterRaw = row[filterColTarget];
      ...
      const filterEmpty = filterRaw === null || filterRaw === undefined || (typeof filterRaw === 'string' && filterRaw.trim() === '');

      // FIX 3: Detect parse errors separately from "missing" values
      ...
      if (!startEmpty) {
        ...
        if (start === null) {
          parseError = `فشل تحليل تاريخ البداية: "${startRaw}"`;
        }
      }
      if (!endEmpty) {
        end = parseDate(endRaw);
        if (end === null && !parseError) {
          parseError = `فشل تحليل تاريخ النهاية: "${endRaw}"`;
        }
      }
      if (!filterEmpty) {
        filterDate = parseDate(filterRaw);
      }

      // Apply date-range filter
      if (hasFilter) {
        if (filterEmpty || !filterDate) { filteredOut++; return; }
        const ms = filterDate.getTime();
        if (fromMs !== null && ms < fromMs) { filteredOut++; return; }
        if (toMs !== null && ms > toMs) { filteredOut++; return; }
      }

      // Parse error (value existed but couldn't be parsed)
      if (parseError) {
        rows.push({
          index: idx, start: null, end: null, startRaw, endRaw,
          diffDays: null, formatted: 'خطأ في الصيغة', status: 'parse_error', parseError,
        });
        parseErrorCount++;
        return;
      }
      ...
      if (!start || !end) {
        rows.push({
          index: idx, start, end, startRaw, endRaw,
          diffDays: null, formatted: 'غير متوفر', status: 'missing',
        });
        missingCount++;
        return;
      }
      ...
      // FIX 2: Invalid = end before start
      if (diffDays < 0) {
        rows.push({
          index: idx, start, end, startRaw, endRaw,
          diffDays, formatted: 'تاريخ غير صحيح', status: 'invalid',
        });
        invalidCount++;
        return;
      }
      ...
      rows.push({
        index: idx, start, end, startRaw, endRaw, diffDays,
    ...
    if (validDiffs.length === 0) {
      return {
        rows, total: rows.length, validCount: 0,
        missingCount, invalidCount, parseErrorCount, filteredOut,
    ...
    return {
      rows, total: rows.length, validCount: validDiffs.length,
      missingCount, invalidCount, parseErrorCount, filteredOut,
      avgDays, minDays: Math.min(...validDiffs), maxDays: Math.max(...validDiffs),
    };
  }, [durationCalculated, startDateCol, endDateCol, data, filterFrom, filterTo, filterDateCol]);
  ...
  // FIX 4: PDF with smart pagination using jsPDF directly (no html2canvas row-cutting)
  const generatePDF = async () => {
    ...
    try {
      ...
      const drawPageHeader = (y: number): number => {
        ...
        doc.rect(margin, y, usableWidth, 1.2, 'F');
        doc.setFont('Helvetica', 'bold');
        doc.setFontSize(20);
        doc.setTextColor(15, 23, 42);
        doc.text('تقرير الإحصائيات', pageWidth / 2, y + 14, { align: 'center' });
        doc.setFontSize(9);
        doc.setTextColor(100, 116, 139);
        doc.text(new Date().toLocaleDateString('ar-EG'), pageWidth / 2, y + 21, { align: 'center' });
        return y + 30;
      };

      // --- Helper: draw section title ---
      const drawSectionTitle = (title: string, cursorY: number, color: [number,number,number] = [79, 70, 229]): number => {
        doc.setFillColor(color[0], color[1], color[2]);
        doc.roundedRect(margin, cursorY, 4, 10, 1, 1, 'F');
        doc.setFont('Helvetica', 'bold');
        doc.setFontSize(13);
        doc.setTextColor(15, 23, 42);
        doc.text(title, pageWidth - margin - 6, cursorY + 7, { align: 'right' });
        return cursorY + 16;
      };

      // --- Helper: summary card grid ---
      const drawSummaryCards = (
        cards: Array<{ label: string; value: string | number; unit?: string; bg: [number,number,number]; fg: [number,number,number] }>,
        cursorY: number
      ): number => {
        const cardW = (usableWidth - (cards.length - 1) * 4) / cards.length;
        cards.forEach((card, i) => {
          const x = margin + i * (cardW + 4);
          doc.setFillColor(card.bg[0], card.bg[1], card.bg[2]);
          doc.roundedRect(x, cursorY, cardW, 22, 3, 3, 'F');
          doc.setFont('Helvetica', 'bold');
          doc.setFontSize(7);
          doc.setTextColor(card.fg[0], card.fg[1], card.fg[2]);
          doc.text(card.label, x + cardW / 2, cursorY + 7, { align: 'center' });
          doc.setFontSize(14);
          doc.setTextColor(15, 23, 42);
          doc.text(String(card.value), x + cardW / 2, cursorY + 16, { align: 'center' });
          if (card.unit) {
            doc.setFontSize(7);
            doc.setTextColor(card.fg[0], card.fg[1], card.fg[2]);
            doc.text(card.unit, x + cardW / 2 + 8, cursorY + 16);
          }
        });
        return cursorY + 28;
      };

      // --- Helper: draw distribution table with smart pagination ---
      const drawTable = (opts: {
        headers: string[];
        rows: Array<{ cells: Array<{ text: string; color?: [number,number,number]; highlight?: 'error' }> ; _bg?: [number,number,number] }>;
        colWidths: number[];
        cursorY: number;
        headerBg: [number,number,number];
        accentColor: [number,number,number];
      }): number => {
        let { headers, rows, colWidths, cursorY, headerBg, accentColor } = opts;
        const rowH = 9;
        const hdrH = 10;
        const tableX = margin;
        ...
        const drawHeader = (y: number): number => {
          doc.setFillColor(headerBg[0], headerBg[1], headerBg[2]);
          doc.roundedRect(tableX, y, tableWidth, hdrH, 2, 2, 'F');
          doc.setFont('Helvetica', 'bold');
          doc.setFontSize(8);
          doc.setTextColor(accentColor[0], accentColor[1], accentColor[2]);
          // Draw headers RTL: start from right
          let x = tableX + tableWidth;
          for (let i = 0; i < headers.length; i++) {
            x -= colWidths[i];
            doc.text(headers[i], x + colWidths[i] / 2, y + hdrH / 2 + 2.5, { align: 'center' });
          }
          return y + hdrH;
        };

        // Ensure header fits on current page
        if (cursorY + hdrH + rowH > pageHeight - margin - 10) {
          doc.addPage();
          cursorY = margin;
        }
        ...
        for (let ri = 0; ri < rows.length; ri++) {
          ...
          // Page break before drawing row if it won't fit
          if (cursorY + rowH > pageHeight - margin - 10) {
            doc.addPage();
            cursorY = margin;
            cursorY = drawHeader(cursorY); // Redraw header on new page
          }

          // Alternating row background
          const defaultBg: [number,number,number] = ri % 2 === 0 ? [255, 255, 255] : [248, 250, 252];
          const bg = row._bg || defaultBg;
          doc.setFillColor(bg[0], bg[1], bg[2]);
          doc.rect(tableX, cursorY, tableWidth, rowH, 'F');

          // Row divider
          doc.setDrawColor(241, 245, 249);
          doc.setLineWidth(0.2);
          doc.line(tableX, cursorY + rowH, tableX + tableWidth, cursorY + rowH);
          ...
          for (let ci = 0; ci < row.cells.length; ci++) {
            ...
            // Red border highlight for invalid date cells
            if (cell.highlight === 'error') {
              doc.setDrawColor(239, 68, 68);
              doc.setLineWidth(0.8);
              doc.rect(x + 0.5, cursorY + 0.5, colWidths[ci] - 1, rowH - 1, 'S');
              doc.setDrawColor(241, 245, 249);
              doc.setLineWidth(0.2);
            }

            doc.text(String(cell.text ?? ''), x + colWidths[ci] / 2, cursorY + rowH / 2 + 2.5, { align: 'center' });
          }
          cursorY += rowH;
        }
      ...
      let cursorY = drawPageHeader(margin);
      cursorY += 6;

      // ---- STATISTICS SECTION ----
      if (activePdfSections.has('statistics') && stats) {
        cursorY = drawSectionTitle('التحليلات والمؤشرات الرقمية', cursorY);

        if (!stats.isComparison) {
          // Summary cards
          const maxVal = Math.max(...stats.values);
          const maxIdx = stats.values.indexOf(maxVal);
          cursorY = drawSummaryCards([
            { label: 'إجمالي السجلات', value: stats.total, bg: [238, 242, 255], fg: [99, 102, 241] },
            { label: 'عدد الفئات', value: stats.labels.length, bg: [253, 242, 248], fg: [236, 72, 153] },
            { label: 'أعلى نسبة', value: `${stats.percentages[maxIdx]}%`, bg: [240, 253, 244], fg: [16, 185, 129] },
          ], cursorY);
          cursorY += 4;

          // Distribution table
          const distRows = stats.labels.map((label, i) => ({
            cells: [
              { text: label, color: [15, 23, 42] as [number,number,number] },
              { text: String(stats.values[i]), color: [71, 85, 105] as [number,number,number] },
              { text: `${stats.percentages[i]}%`, color: [79, 70, 229] as [number,number,number] },
            ]
          }));

          cursorY = drawTable({
            headers: ['%', 'التكرار', 'الفئة'],
            rows: distRows,
            colWidths: [40, 40, usableWidth - 80],
            cursorY,
          ...
          cursorY = drawSummaryCards([
            { label: compareCol1, value: stats.values[0], bg: [240, 253, 244], fg: [16, 185, 129] },
            { label: compareCol2, value: stats.values[1], bg: [238, 242, 255], fg: [99, 102, 241] },
            { label: 'الفارق', value: Math.abs(stats.values[0] - stats.values[1]), bg: [255, 251, 235], fg: [245, 158, 11] },
            { label: 'الإجمالي', value: stats.total, bg: [248, 250, 252], fg: [100, 116, 139] },
          ], cursorY);
        }
        cursorY += 8;
      }
      ...
      // ---- DURATION SECTION ----
      if ((activePdfSections.has('duration_summary') || activePdfSections.has('duration_table')) && durationStats) {
        // Ensure section starts with decent space
        if (cursorY > pageHeight - 80) { doc.addPage(); cursorY = margin; }

        cursorY = drawSectionTitle('تحليل المدة بين تاريخين', cursorY, [79, 70, 229]);

        // Subtitle: which columns
        doc.setFont('Helvetica', 'normal');
        doc.setFontSize(8);
        doc.setTextColor(100, 116, 139);
        doc.text(`${startDateCol}  ←  ${endDateCol}`, pageWidth - margin, cursorY, { align: 'right' });
        cursorY += 8;

        if (activePdfSections.has('duration_summary')) {
          cursorY = drawSummaryCards([
            { label: 'العدد الإجمالي', value: durationStats.total, unit: 'صف', bg: [238, 242, 255], fg: [99, 102, 241] },
            { label: 'متوسط المدة', value: durationStats.avgDays, unit: 'يوم', bg: [219, 234, 254], fg: [59, 130, 246] },
            { label: 'أقصر مدة', value: durationStats.minDays, unit: 'يوم', bg: [240, 253, 244], fg: [16, 185, 129] },
            { label: 'أطول مدة', value: durationStats.maxDays, unit: 'يوم', bg: [254, 243, 199], fg: [245, 158, 11] },
          ], cursorY);
          cursorY += 4;

          // Warning row for invalid/missing
          if (durationStats.invalidCount > 0 || durationStats.missingCount > 0) {
            doc.setFillColor(254, 242, 242);
            doc.roundedRect(margin, cursorY, usableWidth, 10, 2, 2, 'F');
            doc.setFontSize(8);
            doc.setTextColor(153, 27, 27);
            const warnText = [
              durationStats.invalidCount > 0 ? `${durationStats.invalidCount} صف: تاريخ نهاية قبل البداية` : '',
              durationStats.missingCount > 0 ? `${durationStats.missingCount} صف: تاريخ غير متوفر` : '',
            ].filter(Boolean).join('  •  ');
            doc.text(warnText, pageWidth / 2, cursorY + 6.5, { align: 'center' });
            cursorY += 16;
          }
        }

        if (activePdfSections.has('duration_table')) {
          cursorY += 4;

          // Build rows — highlight invalid rows red, color cells accordingly
          const tableRows = durationStats.rows.map(r => {
            const isInvalid = r.status === 'invalid';
            const isMissing = r.status === 'missing';
            const isParseErr = r.status === 'parse_error';
            const bg: [number,number,number] | undefined = isInvalid ? [254, 242, 242] : isMissing ? [250, 250, 250] : undefined;

            return {
              _bg: bg,
              cells: [
                // Row index
                { text: String(r.index + 1), color: [203, 213, 225] as [number,number,number] },
                // Start date cell — red border if invalid
                {
                  text: r.start ? formatDate(r.start) : (r.startRaw ? '؟' : '—'),
                  color: (isInvalid || isParseErr) ? [220, 38, 38] as [number,number,number] : [71, 85, 105] as [number,number,number],
                  highlight: (isInvalid || isParseErr) ? 'error' as const : undefined,
                },
                // End date cell — red border if invalid
                {
                  text: r.end ? formatDate(r.end) : (r.endRaw ? '؟' : '—'),
                  color: (isInvalid || isParseErr) ? [220, 38, 38] as [number,number,number] : [71, 85, 105] as [number,number,number],
                  highlight: (isInvalid || isParseErr) ? 'error' as const : undefined,
                },
                // Duration
                {
                  text: r.formatted,
                  color: (isInvalid || isParseErr) ? [220, 38, 38] as [number,number,number] :
                    isMissing ? [148, 163, 184] as [number,number,number] :
          ...
          cursorY = drawTable({
            headers: ['المدة', endDateCol, startDateCol, '#'],
            rows: tableRows,
            colWidths: [35, 43, 43, 14],
            cursorY,
      ...
      // ---- CROSS-SHEET SECTION ----
      if (activePdfSections.has('cross_sheet') && crossSheetResult) {
        if (cursorY > pageHeight - 80) { doc.addPage(); cursorY = margin; }

        cursorY = drawSectionTitle('مقارنة بين أوراق العمل', cursorY, [16, 185, 129]);
        cursorY = drawSummaryCards([
          { label: `الطرف A — ${crossSheetResult.sheetA}`, value: crossSheetResult.countA, bg: [240, 253, 244], fg: [16, 185, 129] },
          { label: 'الفارق', value: Math.abs(crossSheetResult.countA - crossSheetResult.countB), bg: [248, 250, 252], fg: [100, 116, 139] },
          { label: `الطرف B — ${crossSheetResult.sheetB}`, value: crossSheetResult.countB, bg: [238, 242, 255], fg: [79, 70, 229] },
        ], cursorY);
        cursorY += 4;

        // Distribution tables for both sides
        const distRowsA = Object.entries(crossSheetResult.distA).map(([k, v]) => ({
          cells: [
            { text: k, color: [15, 23, 42] as [number,number,number] },
            { text: String(v), color: [71, 85, 105] as [number,number,number] },
            { text: `${((v / crossSheetResult.totalA) * 100).toFixed(1)}%`, color: [16, 185, 129] as [number,number,number] },
          ]
        }));
        const distRowsB = Object.entries(crossSheetResult.distB).map(([k, v]) => ({
          cells: [
            { text: k, color: [15, 23, 42] as [number,number,number] },
            { text: String(v), color: [71, 85, 105] as [number,number,number] },
            { text: `${((v / crossSheetResult.totalB) * 100).toFixed(1)}%`, color: [79, 70, 229] as [number,number,number] },
          ]
        }));
        ...
        doc.text(`توزيع الطرف A — ${crossSheetResult.colA}`, pageWidth - margin, cursorY, { align: 'right' });
        cursorY += 6;
        cursorY = drawTable({ headers: ['%', 'العدد', 'القيمة'], rows: distRowsA, colWidths: [35, 35, usableWidth - 70], cursorY, headerBg: [236, 253, 245], accentColor: [5, 150, 105] });

        doc.setFontSize(10); doc.setFont('Helvetica', 'bold');
        doc.setTextColor(79, 70, 229);
        doc.text(`توزيع الطرف B — ${crossSheetResult.colB}`, pageWidth - margin, cursorY, { align: 'right' });
        cursorY += 6;
        cursorY = drawTable({ headers: ['%', 'العدد', 'القيمة'], rows: distRowsB, colWidths: [35, 35, usableWidth - 70], cursorY, headerBg: [238, 242, 255], accentColor: [79, 70, 229] });
      }
      ...
      for (let p = 1; p <= totalPages; p++) {
        ...
        doc.text('Jamaa Platform', margin, pageHeight - 4.5);
        doc.text(`${p} / ${totalPages}`, pageWidth - margin, pageHeight - 4.5, { align: 'right' });
        doc.setDrawColor(226, 232, 240);
        doc.setLineWidth(0.3);
        doc.line(margin, pageHeight - 12, pageWidth - margin, pageHeight - 12);
      }

      doc.save(`تقرير-الاحصائيات-${Date.now()}.pdf`);
  ...
  // Available filter columns = all date columns (after start/end are selected, show all date cols)
  const filterableColumns = dateColumns;
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <select value={startDateCol} onChange={(e) => { setStartDateCol(e.target.value); setDurationCalculated(false); if (!filterDateCol) setFilterDateCol(e.target.value); }}
              ...
              {/* FIX 1: Date filter section with column selector */}
              <div className="bg-indigo-50/40 border border-indigo-100 rounded-3xl p-6 space-y-5">
                <div className="flex items-center justify-between flex-wrap gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-bold text-indigo-400">Filtrer par plage de dates</p>
                    </div>
                  </div>
                  {(filterFrom || filterTo) && (
                ...
                {/* ---- NEW: Filter column selector ---- */}
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-indigo-600 uppercase tracking-widest flex items-center gap-2">
                    <CalendarDays size={12} /> طبّق التصفية على العمود / Appliquer le filtre sur la colonne
                  </label>
                  <select
                    value={filterDateCol}
                    onChange={(e) => setFilterDateCol(e.target.value)}
                    className="w-full h-11 px-4 bg-white border-2 border-indigo-100 hover:border-indigo-300 rounded-2xl font-black text-sm text-slate-700 outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all"
                  >
                    {filterableColumns.map(c => (
                      <option key={c} value={c}>
                        {c}{c === startDateCol ? ' (تاريخ البداية)' : c === endDateCol ? ' (تاريخ النهاية)' : ''}
                      </option>
                    ))}
                  </select>
                  <p className="text-[10px] font-bold text-indigo-400">
                    سيتم تصفية السجلات بناءً على قيمة "{filterDateCol || startDateCol}" فقط
                  </p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-indigo-600 uppercase tracking-widest block">من / De</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-indigo-600 uppercase tracking-widest block">إلى / À</label>
                ...
                {(filterFrom || filterTo) && durationCalculated && durationStats && (
                  <div className="text-[11px] font-black text-indigo-600 bg-white px-4 py-2 rounded-2xl border border-indigo-100 inline-flex items-center gap-2">
                    ...
                    <span>تم تصفية {durationStats.filteredOut} صف خارج النطاق • تم احتساب {durationStats.total} صف</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                {durationCalculated && (
                  <button onClick={() => { setDurationCalculated(false); setDurationWarning(null); }}
                    className="flex items-center gap-3 bg-slate-100 text-slate-500 px-6 h-14 rounded-2xl font-black hover:bg-slate-200 transition-all active:scale-95 text-xs">
                    إعادة تعيين
                  </button>
                )}
              </div>
              ...
              <AnimatePresence>
                {durationWarning && (
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                    className="bg-amber-50 border border-amber-100 p-5 rounded-2xl flex items-center gap-3 text-amber-700 font-black text-sm">
          ...
          {durationCalculated && durationStats && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="border-t border-slate-50">
              <div className="p-10 space-y-10">
                ...
                {/* FIX 2 + 3: Duration table with error highlighting */}
                <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden">
                  <div className="p-6 border-b border-slate-50 flex items-center justify-between flex-wrap gap-3">
                    <div>
                      ...
                      <p className="text-xs font-bold text-slate-400 mt-1">
                        ...
                        {durationStats.invalidCount > 0 && ` • ${durationStats.invalidCount} تاريخ غير صحيح`}
                        {(durationStats as any).parseErrorCount > 0 && ` • ${(durationStats as any).parseErrorCount} خطأ في الصيغة`}
                      </p>
                    </div>
                  ...
                  {(durationStats.invalidCount > 0 || (durationStats as any).parseErrorCount > 0) && (
                    <div className="px-6 pt-4 flex flex-wrap gap-3">
                      {durationStats.invalidCount > 0 && (
                        <div className="flex items-center gap-2 text-[10px] font-black text-rose-600 bg-rose-50 px-3 py-1.5 rounded-full border border-rose-100">
                          <div className="w-2.5 h-2.5 rounded-sm bg-rose-200 border-2 border-rose-400" />
                          تاريخ نهاية قبل البداية ({durationStats.invalidCount})
                        </div>
                      )}
                      {(durationStats as any).parseErrorCount > 0 && (
                        <div className="flex items-center gap-2 text-[10px] font-black text-orange-600 bg-orange-50 px-3 py-1.5 rounded-full border border-orange-100">
                          <div className="w-2.5 h-2.5 rounded-sm bg-orange-200 border-2 border-orange-400" />
                          صيغة تاريخ غير صالحة ({(durationStats as any).parseErrorCount})
                        </div>
                      )}
                    </div>
                  )}
                    ...
                    <table className="w-full text-right">
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {durationStats.rows.slice(0, 200).map((r) => {
                          const isInvalid = r.status === 'invalid';
                          const isParseErr = r.status === 'parse_error';
                          const isError = isInvalid || isParseErr;
                          return (
                            <tr key={r.index} className={cn(
                              "hover:bg-slate-50/50 transition-all",
                              isInvalid && "bg-rose-50/60",
                              isParseErr && "bg-orange-50/60",
                              r.status === 'missing' && "bg-slate-50/30"
                            )}>
                              <td className="px-6 py-4 text-xs font-black text-slate-300 font-mono">{r.index + 1}</td>

                              {/* FIX 2: Start date cell — red border + red text when invalid */}
                              <td className={cn("px-6 py-4 text-xs font-mono", isError ? "font-black" : "font-black text-slate-600")}>
                                {isError ? (
                                  <span
                                    title={isParseErr ? "صيغة تاريخ غير صالحة / Format de date invalide" : "تاريخ نهاية قبل البداية"}
                                    className="inline-block px-2 py-0.5 rounded border-2 border-rose-400 bg-rose-50 text-rose-700 cursor-help"
                                  >
                                    {r.start ? formatDate(r.start) : (r.startRaw ? String(r.startRaw) : '—')}
                                  </span>
                                ) : (
                                  r.start ? formatDate(r.start) : <span className="text-slate-300">—</span>
                                )}
                              </td>

                              {/* FIX 2: End date cell — red border + red text when invalid */}
                              <td className={cn("px-6 py-4 text-xs font-mono", isError ? "font-black" : "font-black text-slate-600")}>
                                {isError ? (
                                  <span
                                    title={isParseErr ? "صيغة تاريخ غير صالحة / Format de date invalide" : "تاريخ نهاية قبل البداية"}
                                    className="inline-block px-2 py-0.5 rounded border-2 border-rose-400 bg-rose-50 text-rose-700 cursor-help"
                                  >
                                    {r.end ? formatDate(r.end) : (r.endRaw ? String(r.endRaw) : '—')}
                                  </span>
                                ) : (
                                  r.end ? formatDate(r.end) : <span className="text-slate-300">—</span>
                                )}
                              </td>
                              ...
                              <td className={cn(
                                ...
                                (r.status === 'invalid' || r.status === 'parse_error') && "text-rose-600"
                              )}>
                                {isParseErr ? (
                                  <span title={(r as any).parseError} className="cursor-help flex items-center gap-1">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }}
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button type="button" onClick={() => setPdfSections(new Set(['statistics']))}
                ...
                {durationCalculated && durationStats && (
                  <button type="button" onClick={() => setPdfSections(new Set(['duration_summary', 'duration_table']))}
                ...
                <button type="button" onClick={() => setPdfSections(new Set())}
