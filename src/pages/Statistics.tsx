...
function parseDate(value: any): Date | null {
  ...
  if (sepMatch) {
    ...
    if (a > 31) {
      ...
      if (d) { console.debug(`[parseDate] YYYY/MM/DD "${raw}" → ${d.toISOString()}`); return d; }
    }

    if (a >= 1 && a <= 31 && b >= 1 && b <= 12) {
      const d = makeDate(c, b, a);
      if (d) { console.debug(`[parseDate] DD/MM/YYYY "${raw}" → ${d.toISOString()}`); return d; }
    }

    if (b >= 1 && b <= 31 && a >= 1 && a <= 12) {
      const d = makeDate(c, a, b);
      if (d) { console.debug(`[parseDate] MM/DD/YYYY "${raw}" → ${d.toISOString()}`); return d; }
    }
  }
  ...
  if (isoMatch) {
    ...
    if (d) { console.debug(`[parseDate] ISO "${raw}" → ${d.toISOString()}`); return d; }
  }
...
// ── WESTERN NUMERALS HELPER ──────────────────────────────────────────────────
// Converts any Arabic-Indic (Eastern) digits to ASCII Western digits.
function toWesternNumerals(value: string | number): string {
  return String(value).replace(/[٠١٢٣٤٥٦٧٨٩]/g, (d) =>
    String('٠١٢٣٤٥٦٧٨٩'.indexOf(d))
  );
}

// Returns today's date as dd/mm/yyyy with Western numerals always.
...
export default function Statistics({ data, columns, allSheets }: StatisticsProps) {
  ...
  const [filterDateCol, setFilterDateCol] = useState<string>('');
  ...
  // ============================================================================
  // PDF GENERATION — html2canvas snapshot with proper page margins
  // ============================================================================
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.save(`تقرير-الاحصائيات-${Date.now()}.pdf`);
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="glass-card rounded-[3rem] overflow-hidden">
          <div className="p-10 border-b border-slate-50 flex items-center justify-between">
            ...
            <div className="bg-slate-50 text-slate-400 px-6 py-2 rounded-2xl flex items-center gap-2 font-black text-xs">
              <Calendar size={14} />
              {/* ── Western numerals in the UI table footer date ── */}
              <span>{todayWestern()}</span>
            </div>
          </div>
      ...
      >
        ...
        >
          ...
          <div style={{
            ...
            {/* LEFT — logo (imported asset, always resolves in html2canvas) */}
            ...
            {/* CENTER — title + spacer + date */}
            <div style={{ flex: 1, textAlign: 'center' }}>
              {/* Render title as individual chars in a flex row to avoid html2canvas RTL scramble */}
              <div style={{
                display: 'flex',
          ...
          {activePdfSections.has('statistics') && stats && (
            <div className="space-y-8">
              <h2 className="text-2xl font-black pr-4" style={{ borderRight: '5px solid #4f46e5', color: '#0f172a' }}>
                التحليلات والمؤشرات الرقمية
              </h2>
              ...
              {!stats.isComparison ? (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="p-6 rounded-3xl" style={{ backgroundColor: '#f8fafc' }}>
                      ...
                      <div style={{ height: '300px' }}>
                        <Doughnut data={doughnutData} options={buildDonutOptions({ total: stats.total, isPdf: true, cutout: '55%', valueSize: 13, legendSize: 11 })} />
                      </div>
                    </div>
                    <div className="p-6 rounded-3xl" style={{ backgroundColor: '#f8fafc' }}>
                  ...
                  <div>
                    ...
                    <table className="w-full text-right" style={{ borderCollapse: 'collapse' }}>
                      ...
                      <tbody>
                        {stats.labels.map((label, i) => (
                          <tr key={`${label}-${i}`} style={{ borderBottom: '1px solid #f1f5f9' }}>
                            <td className="px-6 py-3 text-sm font-black" style={{ color: '#0f172a' }}>{label}</td>
                  ...
                  <div className="p-6 rounded-3xl" style={{ backgroundColor: '#eef2ff', border: '1px solid #e0e7ff' }}>
                    ...
                    <p className="text-sm font-bold leading-relaxed" style={{ color: '#1e3a8a' }}>
                      ...
                      تتصدر الفئة "{stats.labels[stats.values.indexOf(Math.max(...stats.values))]}" بنسبة {stats.percentages[stats.values.indexOf(Math.max(...stats.values))]}%،
                      ومتوسط السجلات لكل فئة هو {toWesternNumerals((stats.total / stats.labels.length).toFixed(1))}.
                    </p>
                  </div>
                </>
              ) : (
                <>
                  <div className="grid grid-cols-4 gap-5">
                    <div className="p-6 rounded-3xl text-center" style={{ backgroundColor: '#f0fdf4', border: '1px solid #dcfce7' }}>
                      <p className="text-[10px] font-black" style={{ color: '#10b981' }}>{compareCol1}</p>
                      <p className="text-4xl font-black font-mono mt-3" style={{ color: '#059669' }}>{stats.values[0]}</p>
                    </div>
                    <div className="p-6 rounded-3xl text-center" style={{ backgroundColor: '#eef2ff', border: '1px solid #e0e7ff' }}>
                      <p className="text-[10px] font-black" style={{ color: '#6366f1' }}>{compareCol2}</p>
                      <p className="text-4xl font-black font-mono mt-3" style={{ color: '#4f46e5' }}>{stats.values[1]}</p>
                    </div>
                    <div className="p-6 rounded-3xl text-center" style={{ backgroundColor: '#fffbeb', border: '1px solid #fef3c7' }}>
                      <p className="text-[10px] font-black" style={{ color: '#f59e0b' }}>الفارق</p>
                      <p className="text-4xl font-black font-mono mt-3" style={{ color: '#d97706' }}>{Math.abs(stats.values[0] - stats.values[1])}</p>
                    </div>
                    <div className="p-6 rounded-3xl text-center" style={{ backgroundColor: '#f8fafc', border: '1px solid #e2e8f0' }}>
                      <p className="text-[10px] font-black" style={{ color: '#64748b' }}>الإجمالي</p>
                      <p className="text-4xl font-black font-mono mt-3" style={{ color: '#0f172a' }}>{stats.total}</p>
                    </div>
                  </div>
                  <div className="p-6 rounded-3xl" style={{ backgroundColor: '#f8fafc' }}>
                    <h3 className="text-sm font-black uppercase text-center mb-4" style={{ color: '#94a3b8' }}>المقارنة</h3>
          ...
          {(activePdfSections.has('duration_summary') || activePdfSections.has('duration_table')) && durationStats && (
            <div className="space-y-8" style={{ pageBreakBefore: activePdfSections.has('statistics') ? 'always' : 'auto' }}>
              <h2 className="text-2xl font-black pr-4" style={{ borderRight: '5px solid #4f46e5', color: '#0f172a' }}>
              ...
              {activePdfSections.has('duration_summary') && (
                <>
                  <div className="grid grid-cols-4 gap-5">
                    <div className="p-6 rounded-3xl text-center" style={{ backgroundColor: '#eef2ff', border: '1px solid #e0e7ff' }}>
                      <p className="text-[10px] font-black" style={{ color: '#6366f1' }}>العدد الإجمالي</p>
                      <p className="text-4xl font-black font-mono mt-3" style={{ color: '#4f46e5' }}>{durationStats.total}</p>
                    </div>
                    <div className="p-6 rounded-3xl text-center" style={{ backgroundColor: '#dbeafe', border: '1px solid #bfdbfe' }}>
                      <p className="text-[10px] font-black" style={{ color: '#2563eb' }}>متوسط المدة</p>
                      <p className="text-4xl font-black font-mono mt-3" style={{ color: '#1d4ed8' }}>{durationStats.avgDays}</p>
                    </div>
                    <div className="p-6 rounded-3xl text-center" style={{ backgroundColor: '#f0fdf4', border: '1px solid #dcfce7' }}>
                      <p className="text-[10px] font-black" style={{ color: '#10b981' }}>أقصر مدة</p>
                      <p className="text-4xl font-black font-mono mt-3" style={{ color: '#059669' }}>{durationStats.minDays}</p>
                    </div>
                    <div className="p-6 rounded-3xl text-center" style={{ backgroundColor: '#fef3c7', border: '1px solid #fde68a' }}>
                      <p className="text-[10px] font-black" style={{ color: '#f59e0b' }}>أطول مدة</p>
                      <p className="text-4xl font-black font-mono mt-3" style={{ color: '#d97706' }}>{durationStats.maxDays}</p>
                    </div>
                  </div>

                  {(durationStats.invalidCount > 0 || durationStats.missingCount > 0 || durationStats.parseErrorCount > 0) && (
                    <div className="p-5 rounded-3xl text-sm font-black" style={{ backgroundColor: '#fef2f2', color: '#991b1b', border: '1px solid #fecaca' }}>
                      {durationStats.invalidCount > 0 && <span>{durationStats.invalidCount} صف: تاريخ نهاية قبل البداية. </span>}
                      {durationStats.missingCount > 0 && <span>{durationStats.missingCount} صف: تاريخ غير متوفر. </span>}
                      {durationStats.parseErrorCount > 0 && <span>{durationStats.parseErrorCount} صف: خطأ في صيغة التاريخ.</span>}
                    </div>
                  )}
                </>
              )}
                  ...
                  <table className="w-full text-right" style={{ borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ backgroundColor: '#eef2ff' }}>
                        <th className="px-4 py-3 text-xs font-black" style={{ color: '#4f46e5' }}>#</th>
                        <th className="px-4 py-3 text-xs font-black" style={{ color: '#4f46e5' }}>{startDateCol}</th>
                        <th className="px-4 py-3 text-xs font-black" style={{ color: '#4f46e5' }}>{endDateCol}</th>
                        <th className="px-4 py-3 text-xs font-black" style={{ color: '#4f46e5' }}>المدة</th>
                      </tr>
                    </thead>
                    <tbody>
                      {durationStats.rows.map((r: any) => {
                        const isBad = r.status === 'invalid' || r.status === 'parse_error';
                        return (
                          <tr key={r.index} style={{ borderBottom: '1px solid #f1f5f9', backgroundColor: isBad ? '#fef2f2' : r.status === 'missing' ? '#fafafa' : '#ffffff' }}>
                            <td className="px-4 py-3 text-xs font-black" style={{ color: '#cbd5e1' }}>{r.index + 1}</td>
                            <td className="px-4 py-3 text-xs font-bold" style={{ color: isBad ? '#dc2626' : '#475569' }}>{r.start ? formatDate(r.start) : (r.startRaw ? '؟' : '—')}</td>
                            <td className="px-4 py-3 text-xs font-bold" style={{ color: isBad ? '#dc2626' : '#475569' }}>{r.end ? formatDate(r.end) : (r.endRaw ? '؟' : '—')}</td>
                            <td className="px-4 py-3 text-xs font-black" style={{ color: isBad ? '#dc2626' : r.status === 'missing' ? '#94a3b8' : '#4f46e5' }}>{r.formatted}</td>
                          </tr>
                        );
                      })}
                    </tbody>
          ...
          {activePdfSections.has('cross_sheet') && crossSheetResult && (
            <div className="space-y-8" style={{ pageBreakBefore: (activePdfSections.has('statistics') || activePdfSections.has('duration_summary') || activePdfSections.has('duration_table')) ? 'always' : 'auto' }}>
              <h2 className="text-2xl font-black pr-4" style={{ borderRight: '5px solid #10b981', color: '#0f172a' }}>
                مقارنة بين أوراق العمل
              </h2>

              <div className="grid grid-cols-3 gap-6">
                <div className="p-6 rounded-3xl text-center" style={{ backgroundColor: '#f0fdf4', border: '1px solid #dcfce7' }}>
                  <p className="text-[10px] font-black" style={{ color: '#10b981' }}>الطرف A — {crossSheetResult.sheetA}</p>
                  <p className="text-5xl font-black font-mono mt-3" style={{ color: '#059669' }}>{crossSheetResult.countA}</p>
                </div>
                <div className="p-6 rounded-3xl text-center" style={{ backgroundColor: '#f8fafc', border: '1px solid #e2e8f0' }}>
                  <p className="text-[10px] font-black" style={{ color: '#64748b' }}>الفارق</p>
                  <p className="text-5xl font-black font-mono mt-3" style={{ color: '#0f172a' }}>{Math.abs(crossSheetResult.countA - crossSheetResult.countB)}</p>
                </div>
                <div className="p-6 rounded-3xl text-center" style={{ backgroundColor: '#eef2ff', border: '1px solid #e0e7ff' }}>
                  <p className="text-[10px] font-black" style={{ color: '#4f46e5' }}>الطرف B — {crossSheetResult.sheetB}</p>
                  <p className="text-5xl font-black font-mono mt-3" style={{ color: '#4f46e5' }}>{crossSheetResult.countB}</p>
                </div>
              </div>

              {crossSheetChartData && (
                <div className="p-6 rounded-3xl" style={{ backgroundColor: '#f8fafc' }}>
                  <h3 className="text-sm font-black uppercase text-center mb-4" style={{ color: '#94a3b8' }}>الرسم البياني للمقارنة</h3>
                  <div style={{ height: '320px' }}>
                    <Doughnut data={crossSheetChartData} options={buildDonutOptions({ total: crossSheetResult.grandTotal, isPdf: true, cutout: '55%', valueSize: 13, legendSize: 11 })} />
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <h3 className="text-base font-black mb-4 pr-3" style={{ color: '#059669', borderRight: '4px solid #10b981' }}>
                    توزيع الطرف A — {crossSheetResult.colA}
                  </h3>
                  <table className="w-full text-right" style={{ borderCollapse: 'collapse' }}>
                    <thead><tr style={{ backgroundColor: '#ecfdf5' }}><th className="px-4 py-3 text-xs font-black">القيمة</th><th className="px-4 py-3 text-xs font-black">العدد</th><th className="px-4 py-3 text-xs font-black">النسبة</th></tr></thead>
                    <tbody>
                      {Object.entries(crossSheetResult.distA).map(([k, v]) => (
                        <tr key={k} style={{ borderBottom: '1px solid #f1f5f9' }}>
                          <td className="px-4 py-3 text-xs font-bold">{k}</td>
                          <td className="px-4 py-3 text-xs font-black font-mono">{v as number}</td>
                          <td className="px-4 py-3 text-xs font-black font-mono" style={{ color: '#059669' }}>{(((v as number) / crossSheetResult.totalA) * 100).toFixed(1)}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div>
                  <h3 className="text-base font-black mb-4 pr-3" style={{ color: '#4f46e5', borderRight: '4px solid #4f46e5' }}>
                    توزيع الطرف B — {crossSheetResult.colB}
                  </h3>
                  <table className="w-full text-right" style={{ borderCollapse: 'collapse' }}>
                    <thead><tr style={{ backgroundColor: '#eef2ff' }}><th className="px-4 py-3 text-xs font-black">القيمة</th><th className="px-4 py-3 text-xs font-black">العدد</th><th className="px-4 py-3 text-xs font-black">النسبة</th></tr></thead>
                    <tbody>
                      {Object.entries(crossSheetResult.distB).map(([k, v]) => (
                        <tr key={k} style={{ borderBottom: '1px solid #f1f5f9' }}>
                          <td className="px-4 py-3 text-xs font-bold">{k}</td>
                          <td className="px-4 py-3 text-xs font-black font-mono">{v as number}</td>
                          <td className="px-4 py-3 text-xs font-black font-mono" style={{ color: '#4f46e5' }}>{(((v as number) / crossSheetResult.totalB) * 100).toFixed(1)}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
